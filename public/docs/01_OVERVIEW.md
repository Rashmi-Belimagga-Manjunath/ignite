...
|---|---|
| Exactly five agents | Scout (Researcher) → Muse (Designer) → Forge (Maker) → Voice (Communicator) → Pilot (Manager) |
...
| Tangible output | Forge builds a **working, clickable pop-up website** (rendered live in a sandboxed iframe); the Manager issues a GO / NO-GO decision |
...
1. **Scout** queries Mori Coffee's database, live Dublin weather (Open-Meteo) and live market discussions (Reddit/Wikipedia).
2. **Muse** designs the pop-up concept, experience, menu and brand direction.
3. **Forge** builds the actual pop-up website (live prototype in the interface).
4. **Voice** writes the launch campaign.
5. **Pilot** independently re-queries the live forecast and business numbers, then decides **GO / NO-GO / REVISE** with expected revenue, cost, contribution, confidence and risks.
